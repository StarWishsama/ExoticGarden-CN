...
public class Kitchen extends MultiBlockMachine {
	...
	private RecipeType recipeT = new RecipeType(new CustomItem(Material.CAULDRON, "&e厨房", "", "&a&o这个物品应该在厨房里制作"), "KITCHEN");
	private ExoticGarden plugin;
	
	public Kitchen(ExoticGarden plugin) {
		super(
			Categories.MACHINES_1, new SlimefunItemStack("KITCHEN", Material.CAULDRON, "&e厨房", "", "&a&o可以在此制作许多美味的食物!", "&a&o制作完的食物可以在熔炉处拿到"),
			new ItemStack[] {new CustomItem(Material.BRICK_STAIRS, "&o红砖楼梯 (反着放)"), new CustomItem(Material.BRICK_STAIRS, "&o红砖楼梯 (反着放)"), new ItemStack(Material.BRICKS), new ItemStack(Material.STONE_PRESSURE_PLATE), new ItemStack(Material.IRON_TRAPDOOR), new ItemStack(Material.BOOKSHELF), new ItemStack(Material.FURNACE), new ItemStack(Material.DISPENSER), new ItemStack(Material.CRAFTING_TABLE)},
			new ItemStack[0],
			BlockFace.SELF
		);
		...
		Slimefun.registerResearch(new NamespacedKey(plugin, "kitchen"), 600, "厨房", 30, getItem());
	}
